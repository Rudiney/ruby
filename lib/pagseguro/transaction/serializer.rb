  ...
  class Transaction
    class Serializer
      ...
      def serialize_amounts(data)
        data[:gross_amount] = to_bigdecimal(xml.css("grossAmount").text)
        data[:discount_amount] = to_bigdecimal(xml.css("discountAmount").text)
        data[:net_amount] = to_bigdecimal(xml.css("netAmount").text)
        data[:extra_amount] = to_bigdecimal(xml.css("extraAmount").text)
        data[:installments] = xml.css("installmentCount").text.to_i
      end

      def serialize_creditor(data)
        data[:creditor_fees] = {
          intermediation_rate_amount: to_bigdecimal(xml.css("creditorFees > intermediationRateAmount").text),
          intermediation_fee_amount: to_bigdecimal(xml.css("creditorFees > intermediationFeeAmount").text),
          installment_fee_amount: to_bigdecimal(xml.css("creditorFees > installmentFeeAmount").text),
          operational_fee_amount: to_bigdecimal(xml.css("creditorFees > operationalFeeAmount").text),
          commission_fee_amount: to_bigdecimal(xml.css("creditorFees > commissionFeeAmount").text),
          efrete: to_bigdecimal(xml.css("creditorFees > efrete").text)
        }
      end
      ...
      def serialize_payments(data)
        ...
        xml.css("paymentReleases > paymentRelease").each do |node|
          ...
          payment_release[:total_amount] = to_bigdecimal(node.css("totalAmount").text)
          payment_release[:release_amount] = to_bigdecimal(node.css("releaseAmount").text)
      ...
      def serialize_items(data)
        ...
        xml.css("items > item").each do |node|
          ...
          item[:amount] = to_bigdecimal(node.css("amount").text)
      ...
      def serialize_shipping(data)
        shipping = {
          ...
          cost: to_bigdecimal(xml.css("shipping > cost").text),
        }

...
describe PagSeguro::Transaction::Serializer do
  context "for existing transactions" do
    ...
    it { expect(data).to include(gross_amount: to_bigdecimal("459.50")) }
    it { expect(data).to include(discount_amount: to_bigdecimal("0.00")) }
    it { expect(data).to include(net_amount: to_bigdecimal("445.77")) }
    ...
    it { expect(data[:creditor_fees]).to include(intermediation_rate_amount: to_bigdecimal("0.40")) }
    it { expect(data[:creditor_fees]).to include(intermediation_fee_amount: to_bigdecimal("1644.80")) }
    it { expect(data[:creditor_fees]).to include(installment_fee_amount: to_bigdecimal("0.00")) }
    it { expect(data[:creditor_fees]).to include(operational_fee_amount: to_bigdecimal("0.00")) }
    it { expect(data[:creditor_fees]).to include(commission_fee_amount: to_bigdecimal("1.98")) }
    it { expect(data[:creditor_fees]).to include(efrete: to_bigdecimal("1.98")) }

    it { expect(data[:payment_releases].size).to eq(1) }
    it { expect(data[:payment_releases].first).to include(installment: "1") }
    it { expect(data[:payment_releases].first).to include(total_amount: to_bigdecimal("202.92")) }
    it { expect(data[:payment_releases].first).to include(release_amount: to_bigdecimal("202.92")) }
    ...
    it { expect(data[:items].first).to include(amount: to_bigdecimal("459.50")) }
